...
export const Equipment: React.FC = () => {
    ...
    const getCompareItem = (template: ItemTemplate): ItemInstance | null => {
        ...
        if (template.slot === 'ring') return character.equipment.ring1 || character.equipment.ring2 || null;
        if (template.slot === EquipmentSlot.TwoHand || template.slot === EquipmentSlot.MainHand) return character.equipment.mainHand || character.equipment.twoHand || null;
    ...
    return (
        <ContentPanel title={t('equipment.title')}>
            ...
            {inspectedItem && (
                <ItemTooltip instance={inspectedItem.item} template={inspectedItem.template} affixes={gameData.affixes} character={character} compareWith={getCompareItem(inspectedItem.template)} itemTemplates={gameData.itemTemplates} isCentered={true} onMouseEnter={handleTooltipEnter} onMouseLeave={handleMouseLeave} />
            )}
