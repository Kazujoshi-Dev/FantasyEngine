...
export const ItemTooltip: React.FC<{ instance: ItemInstance; template: ItemTemplate; affixes: Affix[]; character?: PlayerCharacter; compareWith?: ItemInstance | null; x?: number; y?: number; itemTemplates?: ItemTemplate[]; isCentered?: boolean; onClose?: () => void; isSticky?: boolean; }> = ({ instance, template, affixes, character, compareWith, x, y, itemTemplates = [], isCentered, onClose, isSticky }) => { const [style, setStyle] = useState<React.CSSProperties>({ visibility: 'hidden', display: 'none' }); useEffect(() => { if (isCentered || isSticky) { const tooltipWidth = compareWith ? 620 : 300; setStyle({ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', visibility: 'visible', display: 'flex', width: `${tooltipWidth}px`, zIndex: 99999, pointerEvents: (onClose || isSticky) ? 'auto' : 'none' }); return; } if (x === undefined || y === undefined) { setStyle({ visibility: 'inherit', display: 'flex', width: '300px' }); return; } const isSameItem = compareWith?.uniqueId === instance.uniqueId; const tooltipWidth = (compareWith && !isSameItem) ? 620 : 300; let finalX = x + 20; let finalY = y + 20; if (finalX + tooltipWidth > window.innerWidth) finalX = x - tooltipWidth - 20; if (finalY + 450 > window.innerHeight) finalY = window.innerHeight - 470; setStyle({ left: `${finalX}px`, top: `${finalY}px`, visibility: 'visible', display: 'flex', width: `${tooltipWidth}px`, position: 'fixed', zIndex: 9999 }); }, [x, y, compareWith, instance.uniqueId, isCentered, onClose, isSticky]); const isSameItem = compareWith?.uniqueId === instance.uniqueId; const actualCompareWith = isSameItem ? null : compareWith; const compareTemplate = actualCompareWith ? itemTemplates.find(t => t.id === actualCompareWith.templateId) : null; if (isCentered || isSticky) { return ( <div className={`fixed inset-0 z-[99998] flex items-center justify-center p-4 animate-fade-in ${(onClose || isSticky) ? 'bg-black/60 backdrop-blur-sm pointer-events-auto' : 'pointer-events-none'}`} onClick={onClose} > <div className="bg-slate-900/95 border border-slate-700 rounded-xl shadow-2xl p-4 flex gap-4 max-h-[90vh] relative z-[200]" style={{ width: style.width, maxWidth: '95vw', pointerEvents: (onClose || isSticky) ? 'auto' : 'none' }} onClick={e => e.stopPropagation()} > {onClose && ( <button onClick={onClose} className="absolute top-2 right-2 text-gray-500 hover:text-white transition-colors z-20"> ✕ </button> )} {actualCompareWith && compareTemplate && ( <div className="w-[280px] border-r border-white/5 pr-4 hidden md:block overflow-hidden flex flex-col"> <ItemDetailsPanel item={actualCompareWith} template={compareTemplate} affixes={affixes} size="small" compact={true} title="OBECNIE ZAŁOŻONY" itemTemplates={itemTemplates} /> </div> )} <div className={`${actualCompareWith ? 'w-[280px]' : 'w-full'} overflow-hidden flex flex-col`}> <ItemDetailsPanel item={instance} template={template} affixes={affixes} size="small" compact={true} character={character} compareWith={actualCompareWith} itemTemplates={itemTemplates} title={actualCompareWith ? "NOWY PRZEDMIOT" : undefined} /> </div> </div> </div> ); } return ( <div className={`bg-slate-900/95 border border-slate-700 rounded-xl shadow-2xl p-4 pointer-events-none backdrop-blur-md animate-fade-in flex gap-4 ${x !== undefined ? '' : 'absolute bottom-full left-1/2 -translate-x-1/2 mb-4 hidden group-hover:flex'}`} style={style} > {actualCompareWith && compareTemplate && ( <div className="w-[280px] border-r border-white/5 pr-4"> <ItemDetailsPanel item={actualCompareWith} template={compareTemplate} affixes={affixes} size="small" compact={true} title="OBECNIE ZAŁOŻONY" itemTemplates={itemTemplates} /> </div> )} <div className={actualCompareWith ? 'w-[280px]' : 'w-full'}> <ItemDetailsPanel item={instance} template={template} affixes={affixes} size="small" compact={true} character={character} compareWith={actualCompareWith} itemTemplates={itemTemplates} title={actualCompareWith ? "NOWY PRZEDMIOT" : undefined} /> </div> </div> ); };
