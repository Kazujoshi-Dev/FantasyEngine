...
export const ItemTooltip: React.FC<{ instance: ItemInstance; template: ItemTemplate; affixes: Affix[]; character?: PlayerCharacter; compareWith?: ItemInstance | null; x?: number; y?: number; itemTemplates?: ItemTemplate[]; isCentered?: boolean; onClose?: () => void; onMouseEnter?: () => void; onMouseLeave?: () => void; }> = ({ instance, template, affixes, character, compareWith, x, y, itemTemplates = [], isCentered, onClose, onMouseEnter, onMouseLeave }) => { const [style, setStyle] = useState<React.CSSProperties>({ visibility: 'hidden', display: 'none' }); useEffect(() => { if (isCentered) { const tooltipWidth = compareWith ? 620 : 300; setStyle({ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', visibility: 'visible', display: 'flex', width: `${tooltipWidth}px`, zIndex: 99999, pointerEvents: onClose ? 'auto' : 'none' }); return; } if (x === undefined || y === undefined) { setStyle({ visibility: 'inherit', display: 'flex', width: '300px' }); return; } const isSameItem = compareWith?.uniqueId === instance.uniqueId; const tooltipWidth = (compareWith && !isSameItem) ? 620 : 300; let finalX = x + 2; let finalY = y + 2; if (finalX + tooltipWidth > window.innerWidth) finalX = x - tooltipWidth - 10; if (finalY + 450 > window.innerHeight) { finalY = window.innerHeight - 460; if (finalY < 10) finalY = 10; } setStyle({ left: `${finalX}px`, top: `${finalY}px`, visibility: 'visible', display: 'flex', width: `${tooltipWidth}px`, position: 'fixed', zIndex: 9999 }); }, [x, y, compareWith, instance.uniqueId, isCentered, onClose]); const isSameItem = compareWith?.uniqueId === instance.uniqueId; const actualCompareWith = isSameItem ? null : compareWith; const compareTemplate = actualCompareWith ? itemTemplates.find(t => t.id === actualCompareWith.templateId) : null; if (isCentered) { return ( <div className={`fixed inset-0 z-[99998] flex items-center justify-center p-4 animate-fade-in ${onClose ? 'bg-black/60 backdrop-blur-sm pointer-events-auto' : 'pointer-events-none'}`} onClick={onClose} > <div className="bg-slate-900/95 border border-slate-700 rounded-xl shadow-2xl p-4 flex gap-4 max-h-[90vh] relative z-[200] pointer-events-auto" style={{ width: style.width, maxWidth: '95vw' }} onClick={e => e.stopPropagation()} > {onClose && ( <button onClick={onClose} className="absolute top-2 right-2 text-gray-500 hover:text-white transition-colors z-20"> ✕ </button> )} {actualCompareWith && compareTemplate && ( <div className="w-[280px] border-r border-white/5 pr-4 hidden md:flex flex-col max-h-full"> <div className="flex-grow overflow-y-auto custom-scrollbar pr-1"> <ItemDetailsPanel item={actualCompareWith} template={compareTemplate} affixes={affixes} size="small" compact={true} title="OBECNIE ZAŁOŻONY" itemTemplates={itemTemplates} /> </div> </div> )} <div className={`${actualCompareWith ? 'w-[280px]' : 'w-full'} flex flex-col max-h-full`}> <div className="flex-grow overflow-y-auto custom-scrollbar pr-1"> <ItemDetailsPanel item={instance} template={template} affixes={affixes} size="small" compact={true} character={character} compareWith={actualCompareWith} itemTemplates={itemTemplates} title={actualCompareWith ? "NOWY PRZEDMIOT" : undefined} /> </div> </div> </div> </div> ); } return ( <div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={`bg-slate-900/95 border border-slate-700 rounded-xl shadow-2xl pointer-events-auto p-0 backdrop-blur-md animate-fade-in flex gap-4 ${x !== undefined ? '' : 'absolute bottom-full left-1/2 -translate-x-1/2 mb-4 hidden group-hover:flex'}`} style={style} > <div className="relative p-2 flex gap-4 max-h-[85vh] overflow-hidden before:content-[''] before:absolute before:-top-4 before:-left-4 before:-right-4 before:-bottom-4 before:-z-10"> {actualCompareWith && compareTemplate && ( <div className="w-[280px] border-r border-white/5 pr-2 flex flex-col"> <div className="flex-grow overflow-y-auto custom-scrollbar pr-1"> <ItemDetailsPanel item={actualCompareWith} template={compareTemplate} affixes={affixes} size="small" compact={true} title="OBECNIE ZAŁOŻONY" itemTemplates={itemTemplates} /> </div> </div> )} <div className={`${actualCompareWith ? 'w-[280px]' : 'w-full'} flex flex-col`}> <div className="flex-grow overflow-y-auto custom-scrollbar pr-1"> <ItemDetailsPanel item={instance} template={template} affixes={affixes} size="small" compact={true} character={character} compareWith={actualCompareWith} itemTemplates={itemTemplates} title={actualCompareWith ? "NOWY PRZEDMIOT" : undefined} /> </div> </div> </div> </div> ); };
