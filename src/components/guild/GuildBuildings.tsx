...
export const GuildBuildings: React.FC<{ guild: GuildType, myRole: GuildRole | undefined, onUpdate: () => void }> = ({ guild, myRole, onUpdate }) => {
    ...
    const scoutHouseLevel = (guild.buildings && guild.buildings['scoutHouse']) || 0;
    
    const renderBuilding = (type: 'headquarters' | 'armory' | 'barracks' | 'scoutHouse', level: number) => {
        const maxLevel = type === 'barracks' ? 5 : (type === 'scoutHouse' ? 3 : 999);
        ...
        if (type === 'scoutHouse') icon = <MapIcon className="h-8 w-8 text-green-500" />;
        ...
        if (type === 'scoutHouse') effect = `Bonusowe przedmioty: +${level}`;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {renderBuilding('scoutHouse', scoutHouseLevel)}
        </div>
    );
};
