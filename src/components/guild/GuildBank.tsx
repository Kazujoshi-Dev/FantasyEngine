        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-800/50 p-4 rounded-lg flex flex-col h-[600px]">
                <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-bold text-gray-300">Historia Transakcji</h3>
                    <button 
                ...
                <div className="flex-grow overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                    {(guild.transactions || []).length === 0 && <p className="text-gray-500 text-sm text-center py-4">Brak transakcji.</p>}
                    {(guild.transactions || []).map(tx => (
                        <div key={tx.id} className="text-xs bg-slate-900/30 p-2 rounded flex justify-between items-center animate-fade-in">
                            <div>
                                <span className={tx.type === 'DEPOSIT' ? 'text-green-400' : tx.type === 'RENTAL' ? 'text-indigo-400' : tx.type === 'LOOT' ? 'text-amber-500' : 'text-red-400'}>
