...
export const TowerEditor: React.FC<TowerEditorProps> = ({ tower, onSave, onCancel, gameData }) => {
    ...
    // --- Loot Table (Weighted) ---
    const handleLootChange = (floorIndex: number, lootIndex: number, key: keyof LootDrop, value: string) => {
         const floor = formData.floors![floorIndex];
         const updatedLoot = [...(floor.lootTable || [])];
         (updatedLoot[lootIndex] as any)[key] = key === 'weight' ? parseInt(value, 10) || 0 : value;
         updateFloor(floorIndex, { lootTable: updatedLoot });
    };

    const addLootToFloor = (floorIndex: number) => {
         const floor = formData.floors![floorIndex];
         updateFloor(floorIndex, { lootTable: [...(floor.lootTable || []), { templateId: '', weight: 100 }] });
    };
    
    const removeLootFromFloor = (floorIndex: number, lootIndex: number) => {
         const floor = formData.floors![floorIndex];
         updateFloor(floorIndex, { lootTable: (floor.lootTable || []).filter((_, i) => i !== lootIndex) });
    };
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-4">
                    {formData.floors?.map((floor, idx) => {
                        const totalLootWeight = (floor.lootTable || []).reduce((acc, curr) => acc + (curr.weight || 0), 0);
                        const totalResourceWeight = (floor.resourceLootTable || []).reduce((acc, curr) => acc + (curr.weight || 0), 0);
                        ...
                        return (
                            <div key={idx} className="bg-slate-800/50 p-4 rounded border border-slate-700">
                                ...
                                <div className="grid grid-cols-1 gap-3 border-t border-slate-700 pt-2">
                                    ...
                                    {/* Essences (Weighted) */}
                                    <div>
                                         <p className="text-xs text-gray-400 mb-1">Esencje (Suma wag: {totalResourceWeight}):</p>
                                         {floor.resourceLootTable?.map((res, resIdx) => (
                                             <div key={resIdx} className="flex gap-2 items-center text-xs mb-1">
                                                 ...
                                                 <input type="number" placeholder="Waga" value={res.weight} onChange={e => updateEssenceInFloor(idx, resIdx, 'weight', parseInt(e.target.value))} className="w-16 bg-slate-900 p-1 rounded" />
                                                 <span className="text-[10px] text-gray-400 w-10 text-right">
                                                    {totalResourceWeight > 0 ? ((res.weight / totalResourceWeight) * 100).toFixed(1) : 0}%
                                                 </span>
                                     ...
                                     {/* Loot Table (Weighted) */}
                                    <div>
                                         <p className="text-xs text-gray-400 mb-1">Przedmioty (Suma wag: {totalLootWeight}):</p>
                                         {floor.lootTable?.map((loot, lIdx) => (
                                              <div key={lIdx} className="flex gap-2 items-center text-xs mb-1">
                                                 <select value={loot.templateId} onChange={e => handleLootChange(idx, lIdx, 'templateId', e.target.value)} className="w-40 bg-slate-900 p-1 rounded">
                                                     <option value="">Wybierz...</option>
                                                     {gameData.itemTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                                 </select>
                                                 <input type="number" placeholder="Waga" value={loot.weight} onChange={e => handleLootChange(idx, lIdx, 'weight', e.target.value)} className="w-16 bg-slate-900 p-1 rounded" />
                                                 <span className="text-[10px] text-gray-400 w-10 text-right">
                                                    {totalLootWeight > 0 ? ((loot.weight / totalLootWeight) * 100).toFixed(1) : 0}%
                                                 </span>
                                                 <button type="button" onClick={() => removeLootFromFloor(idx, lIdx)} className="text-red-500">X</button>
                                              </div>
                                         ))}
                                         <button type="button" onClick={() => addLootToFloor(idx)} className="text-xs text-purple-400">+ Losowy Przedmiot (Waga)</button>
                                    </div>
                                    
                                    {/* Random Rarity Drops (Independent Chance) - Kept as alternative mechanic */}
                                    <div>
                                         <p className="text-xs text-gray-400 mb-1 mt-2 border-t border-slate-700/30 pt-1">Bonusowe Losowania (Niezależne %):</p>
                                         ...
                                         <button type="button" onClick={() => addRandomItemToFloor(idx)} className="text-xs text-sky-500">+ Bonusowe Rzadkie</button>
                                    </div>
                                </div>
                            </div>
                        );
