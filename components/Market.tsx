...
}> = ({ character, gameData, onCharacterUpdate }) => {
    const { t } = useTranslation();
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
                        <div>
                            ...
                            <select name="slot" value={filters.slot} onChange={handleFilterChange} className="w-full bg-slate-700 p-1.5 rounded-md text-xs">
                                ...
                                {Object.values(EquipmentSlot).map(value => <option key={value} value={value}>{t(`equipment.slot.${value}`)}</option>)}
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="rarity" value={filters.rarity} onChange={handleFilterChange} className="w-full bg-slate-700 p-1.5 rounded-md text-xs">
                                <option value="all">{t('market.browse.filters.all')}</option>
                                {Object.values(ItemRarity).map(value => <option key={value} value={value}>{t(`rarity.${value}`)}</option>)}
                            </select>
                        </div>
                        <div>
...
}> = ({ character, gameData, onCharacterUpdate, onSwitchTab }) => {
    const { t } = useTranslation();
...
export const Market: React.FC<MarketProps> = ({ character, gameData, onCharacterUpdate }) => {
    const { t } = useTranslation();
    ...
    return (
        <ContentPanel title={t('market.title')}>
            ...
            {activeTab === 'browse' && <BrowseListings character={character} gameData={gameData} onCharacterUpdate={onCharacterUpdate} />}
            {activeTab === 'create' && <CreateListing character={character} gameData={gameData} onCharacterUpdate={onCharacterUpdate} onSwitchTab={setActiveTab} />}
            {activeTab === 'my-listings' && <MyListings character={character} gameData={gameData} onCharacterUpdate={onCharacterUpdate} />}
