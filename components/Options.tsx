...
export const Options: React.FC = () => {
  ...
  const hasEmail = !!character.email;
  ...
  const handleSaveProfile = async () => {
    ...
    try {
        ...
        const updatedChar = await api.updateCharacter(profileUpdateData);
        updateCharacter(updatedChar);

        // Then, if email needs to be set, call the separate endpoint
        if (!hasEmail && email.trim()) {
            await api.setEmail(email.trim());
            // Re-fetch character to get the new email reflected
            const finalChar = await api.getCharacter();
        ...
        if (e.message && e.message.toLowerCase().includes('email')) {
