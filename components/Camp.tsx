...
import { api } from '../api';
// FIX: Update import path for getWarehouseUpgradeCost and getWarehouseCapacity
...
const REGEN_INTERVAL_SECONDS = 60;
...
const OverviewPanel: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const timerId = setInterval(() => { 
            setCountdown(prev => {
                if (prev <= 1) {
                    // Refresh character data when countdown hits 0 to sync HP update from server
                    api.getCharacter().then((updated) => updateCharacter(updated));
                    return REGEN_INTERVAL_SECONDS;
                }
                return prev - 1;
            }); 
        }, 1000);
...
const TreasuryPanel: React.FC = () => {
    ...
    const handleDeposit = async (value: number | 'all') => {
        const availableSpace = capacity - chestGold;
        if (availableSpace <= 0) {
        ...
        if (value === 'all') {
             // Deposit all gold but cap at available space
             depositAmount = Math.min(playerGold, availableSpace);
        } else {
             depositAmount = Math.min(playerGold, Number(value));
             // Also prevent overflowing explicitly if user typed a large number
             if (depositAmount > availableSpace) {
                 alert(`Możesz wpłacić maksymalnie ${availableSpace} złota.`);
                 return;
             }
        }
    ...
    const onUpgradeChest = async () => {
        try {
            // Use the correct endpoint for treasury, which is aliased to `upgradeChest` for now
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 bg-slate-900/40 p-4 rounded-xl border border-slate-700/30">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                        Pojemność: <span className="text-white font-bold">{warehouseItems.length} / {capacity}</span>
