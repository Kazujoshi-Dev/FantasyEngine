...
const MainApp: React.FC = () => {
    ...
    const handleCreateCharacter = async (data: { name: string, race: Race }) => {
        try {
            ...
            const created = chars.find(c => c.name === data.name);
            if (created && (created as any).id) {
                 await handleSelectCharacter((created as any).id);
            } else {
                setView('char-select');
            }
        } catch (e: any) {
    ...
    const handleLoginSuccess = (newToken: string) => {
        ...
        setToken(newToken);
        // Effect will trigger init to fetch char list
    };
    ...
    const renderContent = () => {
        switch (activeTab) {
            case Tab.Statistics:
                return <Statistics 
                    character={character} // using direct character since backend handles stats
                    baseCharacter={character} 
                    onCharacterUpdate={handleCharacterUpdate} 
                    calculateDerivedStats={(c) => c} // Pass identity function
                    gameData={gameData}
                    onResetAttributes={() => {}} // Handled in component
            ...
            case Tab.Expedition:
                ...
                />;
            case Tab.Camp:
                // Re-use costs logic from before
                const getCampUpgradeCost = (level: number) => ({ gold: Math.floor(150 * Math.pow(level, 1.5)), essences: [] }); // simplified
                return <Camp 
                    ...
                    getCampUpgradeCost={() => ({ gold: 0, essences: [] })} // UI only, logic in backend
                    onCharacterUpdate={handleCharacterUpdate}
                    ...
                    getChestUpgradeCost={() => ({ gold: 0, essences: [] })}
                    getBackpackUpgradeCost={() => ({ gold: 0, essences: [] })}
                />;
             case Tab.Location:
                return <Location 
