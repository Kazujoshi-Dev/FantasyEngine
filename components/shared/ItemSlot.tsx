...
export const ItemDetailsPanel: React.FC<{
    ...
    const StatSection: React.FC<{title?: string, source: RolledAffixStats | ItemTemplate, isUpgrade: boolean}> = ({title, source, isUpgrade}) => {
        ...
        const entries = [
            ...
            (s.critDamageModifierBonus !== undefined) && {label: t('statistics.critDamageModifier'), value: `+${isUpgrade ? calculateStat(s.critDamageModifierBonus) : s.critDamageModifierBonus}%`},
            (s.armorPenetrationPercent || s.armorPenetrationFlat) && {label: t('statistics.armorPenetration'), value: `${s.armorPenetrationPercent || 0}% / ${isUpgrade ? calculateStat(s.armorPenetrationFlat) : (s.armorPenetrationFlat || 0)}`},
            (s.lifeStealPercent || s.lifeStealFlat) && {label: t('statistics.lifeSteal'), value: `${s.lifeStealPercent || 0}% / ${isUpgrade ? calculateStat(s.lifeStealFlat) : (s.lifeStealFlat || 0)}`},
            (s.manaStealPercent || s.manaStealFlat) && {label: t('statistics.manaSteal'), value: `${s.manaStealPercent || 0}% / ${isUpgrade ? calculateStat(s.manaStealFlat) : (s.manaStealFlat || 0)}`},
    ...
    return (
        <div className="flex flex-col h-full">
            <div className={`flex-grow overflow-y-auto ${isSmall ? 'pr-1' : 'pr-2'}`}>
