...
export const Guild: React.FC<GuildProps> = ({ onCharacterUpdate }) => {
    ...
    const fetchGuild = async () => {
        // Only set loading true on initial load to avoid flashing UI on socket updates
    ...
    useEffect(() => {
        ...
        // FORCED POLLING: Prevents "WebSocket connection failed" errors on production servers
        // where Nginx/Proxy isn't configured to forward Upgrade headers.
        const socket: any = io(socketUrl, {
            transports: ['polling'], 
            upgrade: false, // Explicitly disable upgrade to WebSocket to avoid console errors
        ...
        socket.on('connect', () => {
            // Join the guild room to listen for updates
