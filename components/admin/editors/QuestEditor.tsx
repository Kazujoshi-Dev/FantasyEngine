    ...
    return (
        <form onSubmit={handleSubmit} className="bg-slate-900/40 p-6 rounded-xl mt-6 space-y-6">
            ...
            <fieldset className="grid grid-cols-1 md:grid-cols-3 gap-4 border p-4 rounded-md border-slate-700">
                ...
                {formData.objective?.type !== QuestType.PayGold && <div><label>{t('admin.quest.target')}:<select name="targetId" value={formData.objective?.targetId || ''} onChange={handleObjectiveChange} className="w-full bg-slate-700 p-2 rounded-md mt-1">{getTargetOptions()}</select></label></div>}
