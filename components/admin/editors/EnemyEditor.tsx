        ...
        <form onSubmit={handleSubmit} className="bg-slate-900/40 p-6 rounded-xl mt-6 space-y-6">
            ...
            {/* Magic Stats */}
             <fieldset className="grid grid-cols-2 md:grid-cols-4 gap-4 border p-4 rounded-md border-slate-700">
                ...
                <div><label>{t('admin.enemy.maxMana')}:<input name="maxMana" type="number" value={formData.stats?.maxMana || ''} onChange={handleStatsChange} className="w-full bg-slate-700 p-2 rounded-md mt-1" /></label></div>
                <div><label>{t('admin.enemy.manaRegen')}:<input name="manaRegen" type="number" value={formData.stats?.manaRegen || ''} onChange={handleStatsChange} className="w-full bg-slate-700 p-2 rounded-md mt-1" /></label></div>
                <div><label>{t('admin.enemy.magicDamageMin')}:<input name="magicDamageMin" type="number" value={formData.stats?.magicDamageMin || ''} onChange={handleStatsChange} className="w-full bg-slate-700 p-2 rounded-md mt-1" /></label></div>
                <div><label>{t('admin.enemy.magicDamageMax')}:<input name="magicDamageMax" type="number" value={formData.stats?.magicDamageMax || ''} onChange={handleStatsChange} className="w-full bg-slate-700 p-2 rounded-md mt-1" /></label></div>
                <div><label>{t('admin.enemy.magicAttackChance')}:<input name="magicAttackChance" type="number" value={formData.stats?.magicAttackChance || ''} onChange={handleStatsChange} className="w-full bg-slate-700 p-2 rounded-md mt-1" /></label></div>
                <div><label>{t('admin.enemy.magicAttackManaCost')}:<input name="magicAttackManaCost" type="number" value={formData.stats?.magicAttackManaCost || ''} onChange={handleStatsChange} className="w-full bg-slate-700 p-2 rounded-md mt-1" /></label></div>
                 <div className="md:col-span-2"><label>{t('admin.enemy.magicAttackType')}:<select name="magicAttackType" value={formData.stats?.magicAttackType || ''} onChange={e => handleStatsChange(e as any)} className="w-full bg-slate-700 p-2 rounded-md mt-1"><option value="">-- {t('admin.general.none')} --</option>{Object.values(MagicAttackType).map(v => <option key={v} value={v}>{v}</option>)}</select></label></div>
             </fieldset>
