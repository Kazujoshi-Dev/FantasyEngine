...
export const UniversityTab: React.FC<UniversityTabProps> = ({ skills, onGameDataUpdate }) => {
  ...
  const handleSaveData = (itemFromEditor: Skill | null) => {
    ...
    const itemExists = itemFromEditor.id ? skills.some(d => d.id === itemFromEditor.id) : false;
    let updatedData;

    if (itemExists) {
        updatedData = skills.map(item => item.id === itemFromEditor.id ? itemFromEditor : item);
    } else {
        updatedData = [...(skills || []), { ...itemFromEditor, id: itemFromEditor.id || crypto.randomUUID() }];
    }
          ...
          <div className="space-y-2">
              {(skills || []).map(skill => (
                   <div key={skill.id} className="bg-slate-800/50 p-3 rounded-lg flex justify-between items-center">
                      <div><p className="font-semibold">{skill.name}</p></div>
