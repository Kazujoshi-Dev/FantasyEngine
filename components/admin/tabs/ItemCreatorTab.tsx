...
import { ItemTemplate, Affix, AdminCharacterInfo, ItemCategory, AffixType, ItemInstance, PlayerCharacter } from '../../../types';
import { useTranslation } from '../../../contexts/LanguageContext';
import { api } from '../../../api';
import { ItemDetailsPanel } from '../../shared/ItemSlot';

interface ItemCreatorTabProps {
    users: AdminCharacterInfo[];
    itemTemplates: ItemTemplate[];
    affixes: Affix[];
}

export const ItemCreatorTab: React.FC<ItemCreatorTabProps> = ({ users, itemTemplates, affixes }) => {
    ...
    // Filter affixes by selected template's category (if any selected)
    ...
    const previewItem = useMemo<ItemInstance | null>(() => {
        ...
        // Construct a fake item instance for preview
        // We cannot roll exact stats here easily without duplicating backend logic, 
        // but ItemDetailsPanel handles templates + base stats well.
        // We will simulate "perfect" rolls for preview visual if possible, 
        // but rolledBaseStats is usually required for the panel to show specific numbers.
        // We can mock it with the template's max values.

        const mockRolledStats = (source: any) => {
            const rolled: any = {};
            // Helper to extract max value from {min, max} or value
            ...
            keys.forEach(key => {
                if (source[key] !== undefined) rolled[key] = getMax(source[key]);
            });

            if (source.statsBonus) {
                rolled.statsBonus = {};
                Object.keys(source.statsBonus).forEach(k => {
                     rolled.statsBonus[k] = getMax(source.statsBonus[k]);
                });
            }
            return rolled;
        };
    ...
    const handleSend = async () => {
        ...
        setIsSending(true);
        try {
            await api.adminGiveItem(selectedUserId, {
                templateId: selectedTemplateId,
                prefixId: selectedPrefixId || undefined,
                suffixId: selectedSuffixId || undefined,
                upgradeLevel
