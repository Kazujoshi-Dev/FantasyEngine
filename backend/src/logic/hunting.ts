...
export const processPartyCombat = async (party: HuntingParty, gameData: GameData, client: any): Promise<void> => {
    ...
    for (const member of acceptedMembers) {
        ...
        if (res.rows.length > 0) {
            // Fix: Add missing blockChance property to CharacterStats initialization
             const defaultStats: CharacterStats = {
                ...
                expBonusPercent: 0, goldBonusPercent: 0, damageBonusPercent: 0, damageReductionPercent: number = 0
            };
